...
> **重要注意事项**：有些用户反映在按照上面的方法进行标定时，得到的拼接效果不够好。经过询问，发现原因是他选择的四个标定点都集中在图像的中心部分。你应该尽可能让四个标定点围成的矩形尽可能覆盖图像更大的范围。原因是鱼眼图像在矫正后也是有误差的，边缘的误差更大。所以要尽可能让 OpenCV 在更大的范围内计算一个全局最优的射影矩阵。
